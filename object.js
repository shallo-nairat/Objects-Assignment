...
const  people = [
   ...
   const checkAge = people.filter(item =>{
    return(item.age>=18)
  })
  ...
  { name: 'Headphones', price: 80, category: 'Electronics' },
  { name: 'Shoes', price: 60, category: 'Clothing' },]
  const assembledProducts =Object.groupBy(products,({category}) =>{
  return category;
  })
  ...
  return students.filter(students =>{
      const average =students.scores.reduce(({sum,score}) =>sum +score, 0)/ students.scores.length;
      return average >= 85;
  }).map(students => students.name);
  }
  const highestStudentsAverage =  highestStudentsAverage(students);
  console.log( highestStudentsAverage);
